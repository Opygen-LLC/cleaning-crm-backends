...
export const APP_VERSION: string = process.env.APP_VERSION?.trim() || "1.0.0";
export const GIT_SHA: string = process.env.GIT_SHA?.trim() || process.env.RELEASE_VERSION?.trim() || "unknown";
export const BUILD_DATE: string = process.env.BUILD_DATE?.trim() || "unknown";
export const RELEASE_VERSION: string = process.env.RELEASE_VERSION?.trim() || GIT_SHA;
...
// Signed Next.js Data Cache invalidation. Public mutations call the frontend
// Route Handler directly after commit/cache invalidation; the durable outbox is
// only the retry path when that direct callback times out or is unavailable.
