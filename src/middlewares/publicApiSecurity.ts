...
/** Moderated website reviews are public writes and receive their own budgets. */
export const publicReviewMutationRateLimit = rateLimit({
    ...base,
    limit: 8,
    store: store("public-review-write"),
    message: jsonMessage("Too many review submissions. Please wait and try again."),
});

export const publicReviewResourceRateLimit = rateLimit({
    ...base,
    limit: 120,
    store: store("public-review-resource"),
    keyGenerator: (req: Request) => {
        const key = req.params.identifier ?? "public";
        return `public-review-resource:${String(key).toLowerCase().slice(0, 160)}`;
    },
    message: jsonMessage("This review form is receiving too many submissions. Please try again shortly."),
});
