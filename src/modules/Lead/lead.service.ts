...
const resolveTenantService = async (
  db: LeadServiceDb,
  adminId: string,
...
const buildLeadOperationalWhere = (
  ...
  if (query.assignedToUserId)
    activityWhere.assignedToUserId = String(query.assignedToUserId);
  ...
  if (query.sourceRef)
    where.sourceRef = {
      contains: String(query.sourceRef),
      mode: "insensitive",
    };
  if (query.serviceCatalogId)
    where.serviceCatalogId = String(query.serviceCatalogId);
  ...
  if (query.lastContacted === "never") {
    ...
    where.lastContactedAt =
      query.lastContacted === "stale30" ? { lt: cutoff } : { gte: cutoff };
  }
...
function serializeLead(
  ...
  const projectedBusinessName =
    (
      (lead as Record<string, any>).admin as
        | { businessName?: string }
        | undefined
...
const createLead = async (payload: CreateLeadPayload, user: IRequestUser) => {
  ...
  const phone = payload.phone?.trim()
    ? requireE164Phone(payload.phone, "phone")
    : undefined;
  ...
  const lead = await prisma.$transaction(async (tx) => {
    ...
    if (existing) {
      throw new AppError(
        ...
        "A lead with this email or phone already exists",
        {
          ...
          retryable: false,
          fieldErrors: {
            email: "A lead with this email or phone already exists.",
          },
        },
      );
    }

    const service = await resolveTenantService(
      tx,
      adminProfile.id,
    ...
    const initialStage =
      STAGE_MAP_TO_DB[payload.stage ?? "NEW"] ?? LeadStage.NEW;

    if (payload.initialFollowUp?.assignedToUserId) {
      await ensureLeadActivityAssignee(
        tx,
        adminProfile.id,
        payload.initialFollowUp.assignedToUserId,
      );
    }
...
const deleteLead = async (id: string, user: IRequestUser) => {
  ...
  if (existing.convertedClientId) {
    throw new AppError(
      status.CONFLICT,
      "Converted leads are retained with their activity history",
      {
