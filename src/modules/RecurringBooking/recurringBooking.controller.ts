...
const createSchedule = catchAsync(async (req, res) => {
    const result = await recurringBookingService.createSchedule(req.body, req.user);
    sendResponse(res, {
...
const updateScheduleStatus = catchAsync(async (req, res) => {
    ...
    let result;
    if (action === "pause")   result = await recurringBookingService.pauseSchedule(id, req.user);
    else if (action === "resume") result = await recurringBookingService.resumeSchedule(id, req.user);
...
const deleteSchedule = catchAsync(async (req, res) => {
    await recurringBookingService.deleteSchedule(req.params.id, req.user);
    sendResponse(res, {
        httpStatusCode: status.OK,
        success: true,
