...
const COLORS = {
    ...
    accent:      "#0F172A",   // total-due row background
    accentText:  "#FFFFFF",
    statusPaid:  "#059669",
    statusOver:  "#E11D48",
    statusSent:  "#2563EB",
...
export const generateInvoicePDFBuffer = async (invoiceId: string): Promise<Buffer> => {
    // Fetch invoice with its service catalog
    ...
    return new Promise<Buffer>((resolve, reject) => {
        ...
        // Invoice ref (right-aligned)
        doc
            .font("Helvetica-Bold")
            .fontSize(16)
            .fillColor("#FFFFFF")
            .text(invoice.invoiceRef, 0, 28, { align: "right", width: W - MARGIN });
        ...
        doc
            ...
            .fontSize(8)
            .fillColor(sc)
            .text(invoice.status.toUpperCase(), 0, 54, { align: "right", width: W - MARGIN });
        ...
        const metaItems: Array<{ label: string; value: string; color?: string }> = [
            { label: "ISSUE DATE", value: fmtDate(invoice.issuedDate) },
            {
                ...
                color: invoice.status === "OVERDUE" ? COLORS.statusOver : undefined,
            },
            ...(invoice.paidDate
                ? [{ label: "PAID ON", value: fmtDate(invoice.paidDate), color: COLORS.statusPaid }]
                : []),
            ...(invoice.linkedBookingRef
                ? [{ label: "BOOKING REF", value: invoice.linkedBookingRef }]
                : []),
            ...(invoice.serviceCatalog
                ? [{ label: "SERVICE", value: invoice.serviceCatalog.serviceName }]
                : []),
        ];

        const colW = CONTENT / metaItems.length;
        ...
        const drawPartyBox = (x: number, y: number, w: number, label: string, name: string, lines: string[]) => {
            // Box outline
            doc
                .rect(x, y, w, 80)
        ...
        drawPartyBox(
            ...
            "BILL TO",
            invoice.clientName,
            [invoice.clientEmail, invoice.serviceAddress ?? ""],
        );
        drawPartyBox(
            MARGIN + halfW + 16, BODY_Y, halfW,
            "FROM",
            "CleanCRM Ltd.",
        ...
        if (Number(invoice.taxRate) > 0) {
            drawTotalRow(`VAT (${Number(invoice.taxRate)}%)`, fmt2dp(invoice.taxAmount));
        }
        ...
        if (invoice.notes) {
            ...
            totY += doc.heightOfString(invoice.notes, { width: CONTENT }) + 12;
        }

        // ── 7. Footer ──────────────────────────────────────────────────────────
        const FOOTER_Y = doc.page.height - 56;
        doc.rect(0, FOOTER_Y - 1, W, 1).fill(COLORS.border);
        doc.rect(0, FOOTER_Y, W, 56).fill("#F8FAFC");
        ...
        doc
            ...
            .fillColor(COLORS.muted)
            .text(
                "Questions? accounts@cleancrm.co.uk",
                MARGIN,
                FOOTER_Y + 30,
            );
