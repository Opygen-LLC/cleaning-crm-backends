...
const validateReviewToken = async (token: string) => {
    ...
    if (new Date() > reviewToken.expiresAt)
        throw new AppError(status.GONE, "This review link has expired.");
...
const submitPublicReview = async (
    ...
    if (reviewToken.used)
        throw new AppError(status.GONE, "Review already submitted.");
    if (new Date() > reviewToken.expiresAt)
        throw new AppError(status.GONE, "Review link expired.");
...
const getAllReviews = async (filters: IReviewFilters, user: any) => {
    ...
    const avgRating =
        published.length > 0
            ? Math.round(
                  (published.reduce((s, r) => s + r.rating, 0) /
                      published.length) *
                      10,
              ) / 10
            : 0;
        ...
        stats: {
            ...
            responseRate:
                allForAdmin.length > 0
                    ? Math.round(
                          (allForAdmin.filter((r) => !!r.comment).length /
                              allForAdmin.length) *
                              100,
                      )
                    : 0,
        ...
        data: {
            ...
            ...(payload.isPublished !== undefined
                ? {
                      ...
                      status: payload.isPublished ? "published" : "unpublished",
                  }
                : {}),
            ...(payload.adminReply !== undefined
...
const getStaffReviewSummaries = async (user: any) => {
    ...
    for (const review of staffReviews) {
        ...
        if (review.rating >= 1 && review.rating <= 5)
            entry.ratingCounts[review.rating - 1]++;
        if (review.comment) {
            entry.comments.push({
    ...
    const summaries = Array.from(map.entries()).map(([staffId, data]) => ({
        ...
        averageRating:
            data.total > 0 ? Math.round((data.sum / data.total) * 10) / 10 : 0,
        ratings: [5, 4, 3, 2, 1].map((star) => ({
            star,
            count: data.ratingCounts[star - 1],
        })),
...
const generateTokenForJob = async (jobId: string, _user: any) => {
    ...
    if (job.status !== "COMPLETED") {
        throw new AppError(
            status.BAD_REQUEST,
            "Job must be COMPLETED to generate a review token.",
        );
    }
    ...
    if (client) {
        const staffNames = job.staffAssignments.map(
        ...
        await sendEmailSafely({
            ...
            templateData: {
                ...
                completedDate: new Date(job.scheduledDate).toLocaleDateString(
                    "en-GB",
                    {
                        weekday: "long",
                        day: "numeric",
...
const resendReviewEmail = async (reviewId: string, user: any) => {
    const review = await prisma.review.findUnique({
        where: { id: reviewId },
        select: { jobId: true },
    });
