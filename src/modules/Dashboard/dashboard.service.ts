    ...
    const [
        ...
        prisma.booking.count({
            where: {
                adminId,
                status: {
                    in: [BookingStatus.SCHEDULED, BookingStatus.IN_PROGRESS],
                },
            },
        }),
    ...
    return {
        stats: {
            ...
            avgJobValue: {
                ...
                value:
                    jobCountCurrent > 0
                        ? Math.round(totalRevenue / jobCountCurrent)
                        : 0,
                changePercent: 0,
                prefix: "$",
            },
...
/** Format durationMins → "2h 30m" display string */
...
/** Format a Date to "Apr 25, 09:00am" */
...
const getStaffDashboard = async (userId: string) => {
    ...
    const [
        ...
        // Upcoming jobs (tomorrow + next 7 days)
        prisma.jobStaffAssignment.findMany({
            where: {
                staffId,
                job: {
        ...
        prisma.jobStaffAssignment.count({
            where: {
                staffId,
                job: { scheduledDate: { gte: weekStart, lte: now } },
            },
        }),

        // Jobs previous week (for change%)
        prisma.jobStaffAssignment.count({
            where: {
                staffId,
                job: {
    ...
    const prevWeek = prevWeekJobsCount;
    const weekChangePct =
        prevWeek === 0
            ? 0
            : Math.round(((weekJobsCount - prevWeek) / prevWeek) * 100);
