...
import { BookingStatus, JobStatus, InvoiceStatus } from "../../generated/prisma/enums";
...
const getDashboardOverview = async (userId: string) => {
    ...
    const revenueByDay: Record<string, { revenue: number; jobsCompleted: number; newClients: number }> = {};
...
const getRevenuePage = async (userId: string, period: RevenuePeriod) => {
    ...
    const [
        ...
        prisma.invoice.aggregate({
            where: { adminId, status: InvoiceStatus.PAID, paidDate: { gte: from, lte: now } },
            _sum: { total: true },
        }),
        ...
        prisma.staffProfile.findMany({
            ...
            include: {
                ...
                jobAssignments: {
                    where: {
                        job: { status: JobStatus.COMPLETED, updatedAt: { gte: from, lte: now } },
                    },
                    include: {
    ...
    const jobCountCurrent = await prisma.job.count({
        where: { adminId, status: JobStatus.COMPLETED, updatedAt: { gte: from, lte: now } },
    });
    ...
    return {
        stats: {
            ...
            avgJobValue: {
                label: "Avg. Job Value",
                value: jobCountCurrent > 0 ? Math.round(totalRevenue / jobCountCurrent) : 0,
                changePercent: 0,
                prefix: "$",
            },
            outstandingInvoices: {
