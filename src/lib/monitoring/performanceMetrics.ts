...
export const recordRedisReadMetric = (input: { durationMs: number; hits: number; misses: number; error?: boolean }): void => {
  redis.reads += input.hits + input.misses || 1; redis.hits += input.hits; redis.misses += input.misses; if (input.error) redis.errors += 1;
...
export const getPerformanceSnapshot = () => {
  ...
  return {
    generatedAt: new Date().toISOString(), uptimeSeconds: Math.round(process.uptime()), window: getRecentWindowSnapshot(),
    ...
    redis: { reads: redis.reads, hits: redis.hits, misses: redis.misses, errors: redis.errors, hitRate: percent(redis.hits, redisAttempts), avgMs: average(redis.totalMs, redis.samples.length), p50Ms: percentile(redis.samples, 50), p95Ms: percentile(redis.samples, 95), p99Ms: percentile(redis.samples, 99) },
  };
};
...
export const resetPerformanceMetricsForTests = (): void => {
  ...
  redis.reads = 0; redis.hits = 0; redis.misses = 0; redis.errors = 0; redis.totalMs = 0; redis.samples = [];
};
