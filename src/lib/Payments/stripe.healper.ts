...
interface PaymentData {
    ...
    author_id: string;
    /**
     * The invoice ID.  Stored as `invoiceId` in Stripe session metadata so the
     * webhook handler can read `metadata.invoiceId` directly.
     * The legacy field name `booking_id` is kept for backwards compatibility.
     */
    booking_id: string;
}
 ...
 * @param paymentData - Object containing amount, name, author_id, booking_id (invoice ID), and optional URLs
...
const createStripePaymentLink = async (paymentData: PaymentData) => {
    try {
        ...
        const session = await stripe.checkout.sessions.create({
                    ...
                    price_data: {
                        ...
                        product_data: {
                            ...
                            metadata: {
                                ...
                                // Keep booking_id for product-level tracing; session metadata
                                // uses invoiceId so the webhook handler can find it.
            ...
            metadata: {
                author_id: author_id.toString(),
                // invoiceId is what the webhook handler looks for (metadata.invoiceId).
                // booking_id is kept alongside for legacy compatibility.
                invoiceId: booking_id.toString(),
                booking_id: booking_id.toString(),
            },
        });
