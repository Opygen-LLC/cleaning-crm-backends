...
/**
 * Phase 4 read-only plan audit for the SQL shapes changed by the hot-path work.
 *
 * Usage:
 *   PERF_ADMIN_ID=<admin-id> PERF_DB_TARGET_MS=30 pnpm perf:phase4:explain
 *
 * EXPLAIN ANALYZE executes SELECT statements only. Run against staging/a
 * production-like replica first. This script intentionally does not create
 * indexes: plan evidence must exist before a migration is added.
 */
...
const probes: Probe[] = [
  {
    ...
    sql: `SELECT s.id, s."expiresAt"
          FROM "session" s
          WHERE s."userId" = (SELECT ap."userId" FROM "AdminProfile" ap WHERE ap.id = $1)
            AND s."expiresAt" > NOW()
  ...
  },
  {
    ...
    sql: `SELECT COALESCE(SUM(i.total), 0)
          FROM "invoice" i
          WHERE i."adminId" = $1 AND i.status = 'PAID'
  ...
  },
  {
    ...
    sql: `SELECT b.id, b."scheduledDate"
          FROM "booking" b
          WHERE b."adminId" = $1 AND b.status = 'SCHEDULED'
          ORDER BY b."scheduledDate" DESC, b.id DESC
          LIMIT 50`,
  },
  {
    name: "lead-stage-newest",
    sql: `SELECT l.id, l."createdAt"
          FROM "lead" l
          WHERE l."adminId" = $1 AND l.stage = 'NEW'
          ORDER BY l."createdAt" DESC, l.id DESC
          LIMIT 50`,
  },
  {
    name: "website-owner-row",
    sql: `SELECT bw.id, bw.status, bw."updatedAt"
  ...
  },
  {
    ...
    sql: `SELECT s.id
          FROM "Subscription" s
          WHERE s."adminId" = $1 AND s.status = 'ACTIVE'
          LIMIT 1`,
  },
];
